...
export default function App() {
  ...
  useEffect(function(){
    async function load(){
      try {
        if(window.storage){
          var rd = await window.storage.get("aq7-rows"); if(rd&&rd.value) setRows(JSON.parse(rd.value));
          var ud = await window.storage.get("aq7-users"); if(ud&&ud.value) setUsers(JSON.parse(ud.value));
          var ld = await window.storage.get("aq7-logs"); if(ld&&ld.value) setLogs(JSON.parse(ld.value));
        } else {
          var rd2 = localStorage.getItem("aq7-rows"); if(rd2) setRows(JSON.parse(rd2));
          var ud2 = localStorage.getItem("aq7-users"); if(ud2) setUsers(JSON.parse(ud2));
          var ld2 = localStorage.getItem("aq7-logs"); if(ld2) setLogs(JSON.parse(ld2));
        }
  ...
  var persist = useCallback(function(key,val){try{if(window.storage){window.storage.set(key,JSON.stringify(val));}else{localStorage.setItem(key,JSON.stringify(val));}}catch(e){}},[]);
